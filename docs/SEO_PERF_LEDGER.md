...
**19:30 ET — the blank basemap, root-caused (`7a6b8db`).** The pins showed but the map background never drew: style.json, the tile index and the sprite all fetched 200, yet no source cache was built and **zero `.mvt` tiles were requested**. Cause: `maplibre-gl@6.9.1` (published the same day) did not spawn its Web Worker under Next 16's Turbopack, so the tile pipeline was dead — no CSP, no failed request, silent. Fix: pin `maplibre-gl@5.24.0` (stable). Verified in a rendering browser: vector tiles now load (`.mvt` 200s) and the full Positron basemap draws — Frederick, Baltimore, Washington, the Chesapeake, roads and labels — with the pins on top. Mobile top-of-page Lighthouse **95 / 100 / 100 / 66**, LCP 3.0 s, CLS 0. The earlier speed wins (prefetch, preconnect, no fade, earlier trigger) stay.
