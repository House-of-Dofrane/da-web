// Cookie Consent from Hirael <https://hirael.com/blocks/widgets/cookie-consent>
...
interface CookieConsentProps extends Omit<
...
const CookieConsent = ({
  ...
  // The onKeyDown above only catches Escape while focus is inside the banner, but activating
  // "Manage" unmounts the focused trigger and drops focus to <body> — so a document-level listener
  // is what actually makes Escape collapse the panel for keyboard and mouse users alike.
  React.useEffect(() => {
    if (!expanded) return;
...
const CookieConsentTitle = ({
  className,
  children,
  ...props
}: CookieConsentTitleProps) => {
  return (
    <h2
      data-slot="cookie-consent-title"
      className={cn(
        "flex items-center gap-2 text-sm font-semibold tracking-[-0.01em] text-foreground",
        className,
      )}
      {...props}
    >
...
const CookieConsentDescription = ({
  className,
  ...props
}: CookieConsentDescriptionProps) => {
  return (
    <p
...
const CookieConsentManage = ({
  className,
  children = "Manage",
  ...props
}: CookieConsentManageProps) => {
...
/**
 * Holds the per-category switches. Visible only while expanded, but always mounted so each
 * category registers itself — otherwise "Accept all" / "Reject all" pressed before opening the
 * panel would commit an empty record and no gate would flip. (Adapted from the source, which
 * returned null when collapsed; that left the categories unregistered on the common accept path.)
 */
...
interface CookieConsentCategoryProps extends Omit<
  React.ComponentProps<"div">,
  "children"
> {
...
const CookieConsentAcceptAll = ({
  ...
  return (
    <Button
      type="button"
      size="sm"
      data-slot="cookie-consent-accept-all"
      onClick={acceptAll}
      {...props}
    >
      {children}
